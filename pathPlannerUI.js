...
function initMap() { //google maps initialization
  ...
  google.maps.event.addListener(drawingManager, 'markercomplete', function(marker) { //callback for home point storage on click
    //if lastMarker is not empty, remove it
    //if(lastMarker) {
      //lastMarker.setMap(null);
    //}
    marker.setMap(map); //2-13-2019 Noah: uses Google's API to set a marker, foregoing the grid system for the home point
    setHome(marker,map);
  })
}
...
function setHome(marker, map){ //allows user to select a "home point" which will snap to the grid
  if (!google.maps.geometry.poly.containsLocation(marker.getPosition(),activePoly[0])){
    ...
    marker.setMap(null); //prevent the marker from being created
  } else {
    //if array is not empty, pop off last coord
    if (activeHome.length > 0) {
      activeHome.pop();
    }
    /*//set lastMarker to be removed if marker already exists
    ...
    activeHome.push(marker); //push the coordinates to the home

    ////////////////// FEATURE TO ADD: remove current home point and delete
  }
}
